...
def main():
    # Check if TF SavedModel exists (try fixed version first)
    tf_model_paths = [
        "./models/converted/tensorflow_fixed",
        "./models/converted/tensorflow"
    ]
    
    tf_model_path = None
    for path in tf_model_paths:
        if os.path.exists(path):
            tf_model_path = path
            print(f"Using TF model: {tf_model_path}")
            break
    
    if not tf_model_path:
        print(f"Error: No TensorFlow SavedModel found!")
        print("Available paths checked:")
        for path in tf_model_paths:
            print(f"  - {path}")
        print("Please run: python fix_tf_model_signature.py")
        return
    ...
    print("\nTFLite models saved to ./models/mobile/")
